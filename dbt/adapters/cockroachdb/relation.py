...
from dbt.adapters.cockroachdb.relation_configs import (
...
class CockroachdbRelation(BaseRelation):
    ...
    def __post_init__(self):
        # Check for length of Cockroachdb table/view names.
    ...
    def get_materialized_view_config_change_collection(
    ...
    ) -> Optional[CockroachdbMaterializedViewConfigChangeCollection]:
        config_change_collection = CockroachdbMaterializedViewConfigChangeCollection()

        existing_materialized_view = CockroachdbMaterializedViewConfig.from_relation_results(
            relation_results
        )
        new_materialized_view = CockroachdbMaterializedViewConfig.from_model_node(
        ...
        # we return `None` instead of an empty `CockroachdbMaterializedViewConfigChangeCollection` object
    ...
    def _get_index_config_changes(
        self,
        existing_indexes: FrozenSet[CockroachdbIndexConfig],
        new_indexes: FrozenSet[CockroachdbIndexConfig],
    ) -> Set[CockroachdbIndexConfigChange]:
        """
        ...
        """
        drop_changes = set(
            CockroachdbIndexConfigChange.from_dict(
