...
{% macro cockroachdb__make_relation_with_suffix(base_relation, suffix, dstring) %}
    {% if dstring %}
...
{% macro cockroachdb__make_intermediate_relation(base_relation, suffix) %}
    {{ return(cockroachdb__make_relation_with_suffix(base_relation, suffix, dstring=False)) }}
{% endmacro %}

{% macro cockroachdb__make_temp_relation(base_relation, suffix) %}
    {% set temp_relation = cockroachdb__make_relation_with_suffix(base_relation, suffix, dstring=True) %}
    {{ return(temp_relation.incorporate(path={"schema": none,
                                              "database": none})) }}
{% endmacro %}

{% macro cockroachdb__make_backup_relation(base_relation, backup_relation_type, suffix) %}
    {% set backup_relation = cockroachdb__make_relation_with_suffix(base_relation, suffix, dstring=False) %}
...
{%- macro cockroachdb__get_show_grant_sql(relation) -%}
...
{% macro cockroachdb__copy_grants() %}
    {{ return(False) }}
{% endmacro %}


{% macro cockroachdb__get_show_indexes_sql(relation) %}
    select
        i.relname                                   as name,
        m.amname                                    as method,
...
{%- macro cockroachdb__get_drop_index_sql(relation, index_name) -%}
